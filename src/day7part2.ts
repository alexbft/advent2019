...
class Computer {
    ...
    getInstance() {
        return new ComputerInstance(this.program, []);
    }
}
...
async function main() {
    ...
    for await (const programRaw of f.readLines()) {
        ...
        for (const phases of phasePermutations) {
            const instances = phases.map(phase => {
                const instance = computer.getInstance();
                instance.inputs.push(phase);
                return instance;
            });
            let signal: number | null = 0;
            let lastSignal: number = 0;
            while (signal != null) {
                for (const instance of instances) {
                    if (signal != null) {
                        instance.inputs.push(signal);
                        lastSignal = signal;
                    }
                    signal = instance.runUntilOutput();
                }
            }
            if (maxSignal < lastSignal) {
