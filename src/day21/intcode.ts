...
export function inputFromArray(a: number[]): InputGenerator {
...
interface ExecuteInstructionOptions {
    stopOnInput?: boolean;
}
...
export class Computer {
    ...
    run(inputs: InputGenerator): number[] {
...
export class ComputerInstance {
    ...
    executeNextInstruction(options?: ExecuteInstructionOptions): ExecuteInstructionResult {
        if (this.isHalted) {
        ...
        switch (instruction.name) {
            ...
            case 'input': { // store input value
                if (options?.stopOnInput) {
                    this.iptr -= instruction.length;
                    return { requireInput: true };
                }
                const input = this.inputs();
                //console.log(`Input: ${input}`);
                this.writeMemory(params[0], input);
                break;
            }
            case 'output': { // output
                output = params[0];
    ...
    async runWithInputGenerator(inputs: AsyncGenerator<number, void, number[]>): Promise<number[]> {
        let outputBuffer: number[] = [];
        const inputBuffer: number[] = [];
        this.inputs = inputFromArray(inputBuffer);
        while (!this.isHalted) {
            const result = this.executeNextInstruction({ stopOnInput: true });
            if (result.output != null) {
                outputBuffer.push(result.output);
            }
            if (result.requireInput) {
                const genResult = await inputs.next(outputBuffer);
                outputBuffer = [];
                if (genResult.done) {
                    throw new Error(`Input required`);
                }
                inputBuffer.push(genResult.value);
                const result2 = this.executeNextInstruction();
